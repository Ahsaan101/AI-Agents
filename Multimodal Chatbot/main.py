...
client = OpenAI()
def multimodal_agent(image_url=None, query=None, audio_transcript=None):
    message_list = []
...
async def chat(msg: cl.Message):
    ...
    if len(images) == 0 and len(audios) == 0:
        response = multimodal_agent(query=msg.content)

    elif len(audios) == 0:
        response = multimodal_agent(image_url=image_url, query=msg.content)

    else:
        response = multimodal_agent(query=msg.content, audio_transcript=text)
